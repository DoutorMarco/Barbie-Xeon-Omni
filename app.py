...
# [PROTOCOL 00: TENTATIVA DE INSTALAÇÃO DE ELITE]
try:
    from pyvis.network import Network
    import streamlit.components.v1 as components
    PYVIS_ACTIVE = True
except ImportError:
    os.system(f"{sys.executable} -m pip install pyvis biopython fpdf2 httpx")
    PYVIS_ACTIVE = False # Backup para visualização nativa se falhar
...
def get_script_integrity():
    ...
    except: return "XEON_v101_28_ULTIMATE_SHIELD"

SCRIPT_HASH = get_script_integrity()
VALOR_HORA = 1000.00 

# [PROTOCOL 02: ESTÉTICA BLACKOUT TOTAL BLINDADA]
st.set_page_config(page_title="XEON OMNI v101.28", layout="wide")
st.markdown("""
    <style>
...
with t2:
    st.subheader("🧬 Grafo de Conhecimento & Habilidade Extraordinária")
    if PYVIS_ACTIVE:
        ...
        # BACKUP SOBERANO: Gráfico Nativo Plotly se a biblioteca falhar
        st.warning("MODO DE CONTINGÊNCIA: Visualizador Nativo Ativado.")
        fig_net = go.Figure(go.Scatter(x=[0, 1, -1], y=[0, 1, 1], mode='markers+text+lines',
                                     text=["XEON", "BIO", "DEFESA"], marker=dict(size=40, color="#00FF41")))
        fig_net.update_layout(paper_bgcolor="black", plot_bgcolor="black", showlegend=False)
        st.plotly_chart(fig_net, use_container_width=True)

with t4:
    st.subheader("⚙️ Depurador e Gerador de Dossiê EB-1A")
    def generate_pdf_v28():
        pdf = FPDF(); pdf.set_text_color(0, 255, 65)
        for _ in range(5):
            pdf.add_page(); pdf.set_fill_color(0); pdf.rect(0,0,210,297,'F')
            pdf.set_font("Courier","B",16); pdf.cell(0,10,"XEON OMNI COMMAND - DOSSIER v101.28",0,1,'C')
            pdf.set_font("Courier","",10); pdf.multi_cell(0,8,f"ARQUITETO: MARCO ANTONIO\nRECEITA: R$ {revenue:.2f}\nHASH: {SCRIPT_HASH}")
        return bytes(pdf.output())

    st.download_button("💾 BAIXAR DOSSIÊ MASTER (PDF 5 FOLHAS)", data=generate_pdf_v28(), file_name="XEON_OMNI_AUDIT.pdf")

st.chat_input("Sistema blindado contra falhas de biblioteca. Auditoria Nominal.")
