...
st.components.v1.html("""
    <div style="display:flex; gap:10px; margin-bottom:20px;">
        <button onclick="window.speechSynthesis.speak(new SpeechSynthesisUtterance('Sistema Nominal. Versão 100 operacional.'))" style="flex:1; background:black; color:#00FF41; border:1px solid #00FF41; padding:15px; cursor:pointer; font-family:monospace; font-weight:bold;">🔊 VOZ ON</button>
        <button onclick="alert('🎙️ Escuta neural ativa. Aguardando comando.')" style="flex:1; background:black; color:#00FF41; border:1px solid #00FF41; padding:15px; cursor:pointer; font-family:monospace; font-weight:bold;">🎙️ MIC ON</button>
    </div>
""", height=80)

# [PROTOCOL 04: KERNEL DE DADOS REALIDADE PURA - FIX 403/0.0]
if 'kernel_v100' not in st.session_state:
    st.session_state.ledger_cache = []
    st.session_state.kernel_v100 = True

async def fetch_real_intel():
    # Headers para evitar bloqueio 403 e erro R$ 0.0
...
with t2:
    ...
    with c_b:
        if st.button("🚀 INFILTRAR"):
            payload = {"p": price, "geo": geo, "ts": str(time.time())}
        ...
        if st.button("📄 PREPARAR PDF"):
            ...
            pdf.cell(0, 10, "XEON v100 - AUDIT REPORT", 0, 1, 'C')
            pdf.set_font("Helvetica", "", 10); pdf.ln(10)
            content = f"DATA: {datetime.datetime.now()}\nIP: {geo.get('query')}\nPRICE: R$ {price}\nHASH: {SCRIPT_HASH}"
            pdf.multi_cell(0, 7, content)
            
            # [FIX DEFINITIVO PARA DOWNLOAD_BUTTON ERROR]
            pdf_output = pdf.output()
            # fpdf2 retorna bytes se nenhum nome de arquivo for passado, mas garantimos aqui:
            pdf_bytes = bytes(pdf_output) if isinstance(pdf_output, (bytearray, bytes)) else pdf_output.encode('latin-1')
            
            st.download_button("💾 BAIXAR AGORA", data=pdf_bytes, file_name="XEON_v100.pdf", mime="application/pdf")
...
st.chat_input("Operação v100 nominal...")
