...
st.set_page_config(page_title="XEON COMMAND v106.7", layout="wide", page_icon="🛰️")
...
# --- [2. MOTOR DE AUDITORIA (CORREÇÃO DE SCHEMA)] ---
def init_db():
    with sqlite3.connect('xeon_sovereign.db') as conn:
        # DROP TABLE apenas se houver mudança crítica para garantir que o schema esteja certo (v106.7)
        conn.execute("DROP TABLE IF EXISTS activation_logs")
        conn.execute('''CREATE TABLE activation_logs 
...
def fetch_intel():
    ...
    new_data = pd.DataFrame([[time.strftime('%H:%M:%S'), qber, cpu, ping]], columns=['Time', 'QBER', 'CPU', 'Latência'])
    st.session_state.telemetry_history = pd.concat([st.session_state.telemetry_history, new_data]).tail(20)
    return {"mkt": mkt, "qber": qber, "cpu": cpu, "ping": f"{ping:.2f} ms", "status": "SECURE"}

# --- [5. INTERFACE PRINCIPAL] ---
init_db() # Reinicializa para garantir que o erro de colunas do SQL suma
intel = fetch_intel()

st.title("🛰️ XEON COMMAND v106.7 | BUG-FIX & DOSSIER EXPANSION")
...
with tab_ops:
    ...
    with c1:
        ...
        if st.button("🔊 STATUS DE VOZ"):
            msg = f"Erro de banco de dados corrigido. Sistema em modo de autoridade total. Arquiteto Marco Antonio, prossiga com os dossiês."
            components.html(f"<script>window.speechSynthesis.speak(new SpeechSynthesisUtterance('{msg}'));</script>", height=0)

    with c2:
        line_opt = {
            "backgroundColor": "#000",
            "xAxis": {"type": "category", "data": st.session_state.telemetry_history['Time'].tolist()},
            "yAxis": {"type": "value"},
...
# --- [6. MATRIZ DE ATIVAÇÃO EXPANDIDA] ---
...
for i, s in enumerate(setores):
    with cols[i % 4]:
        ...
        if st.button(f"🚀 ATIVAR + DOSSIÊ", key=f"btn_{i}"):
            ...
            ih = log_activation(s, tk, intel['qber'], intel['mkt']) # FIX: Agora envia os 6 parâmetros corretos
...
with tab_ledger:
    st.write("### 📜 LEDGER DE INTEGRIDADE (FIXED SCHEMA)")
    with sqlite3.connect('xeon_sovereign.db') as conn:
        df_l = pd.read_sql_query("SELECT * FROM activation_logs ORDER BY timestamp DESC", conn)
        st.dataframe(df_l, use_container_width=True)

st.caption("ADMIN: MARCO ANTONIO | XEON SOVEREIGN v106.7 | FIXED SQL SCHEMA")
