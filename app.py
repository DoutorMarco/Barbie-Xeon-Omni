...
st.set_page_config(page_title="XEON COMMAND v124.0", layout="wide", page_icon="🛰️")

# CSS AGRESSIVO: ELIMINAÇÃO TOTAL DE BRANCO E AZUL
st.markdown(f"""
    <style>
    ...
    .stButton button:hover, .stDownloadButton button:hover {{ background-color: {MATRIX_GREEN} !important; color: {BLACKOUT} !important; box-shadow: 0 0 25px {MATRIX_GREEN}; }}
...
# --- [2. MOTOR DE AUDITORIA E DOCUMENTAÇÃO (CORREÇÃO DE ATRIBUTO)] ---
def init_db():
...
def generate_node_pdf(sector, cpu, mkt, i_hash):
    """Gera PDF Funcional Corrigido para fpdf2."""
    pdf = FPDF()
    pdf.add_page()
    ...
    # fpdf2 retorna bytes diretamente quando output() é chamado sem argumentos
...
def operational_core():
    ...
    with c_left: 
        ...
        if st.button("🔥 ATIVAÇÃO GLOBAL (OMNI)"):
            ...
            components.html(f"<script>window.speechSynthesis.speak(new SpeechSynthesisUtterance('Ciclo Global Realizado.'));</script>", height=0)
    ...
    # GRADE DE NÓS COM PDFs FUNCIONAIS REGENERADOS
    cols = st.columns(4)
...
# --- [4. EXECUÇÃO E LEDGER] ---
init_db()
st.title("🛰️ XEON COMMAND v124.0 | OMNI-DOCS FIXED")
operational_core()
...
with sqlite3.connect('xeon_sovereign_v124.db') as conn:
    ...
    if not df_ledger.empty:
        ...
        st.info("SISTEMA OPERACIONAL. AGUARDANDO COMANDO DE ATIVAÇÃO.")
